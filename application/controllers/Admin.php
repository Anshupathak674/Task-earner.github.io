...
class  Admin extends CI_Controller
{
  ...
  function add_to_cart()
  {
    ...
    if ($this->upload->do_upload('project_ss')) {
        $data =  $this->upload->data();
        $project_ss = $data['raw_name'] . $data['file_ext'];
        $post['project_ss'] = $project_ss;
    }

    $this->Admin_model->add_to_cart($project_ss);

    $add_id = $this->input->post('add_id');
    $this->session->set_flashdata('msg', 'Task Submitted');
